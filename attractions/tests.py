...
class SaveAuthGateTest(TestCase):
    ...
    def test_authenticated_post_saves_and_redirects(self):
        ...
        self.assertTrue(UserSavedAttraction.objects.filter(user=self.user, attraction=self.attraction).exists())
...
class UnsaveViewTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.attraction = make_attraction("Unsave Test Attraction")
        cls.user = User.objects.create_user("unsaveuser", password="testpass123!")
        cls.save_record = UserSavedAttraction.objects.create(user=cls.user, attraction=cls.attraction)

    def test_anonymous_post_redirects_to_login(self):
        url = reverse("attraction-unsave", args=[self.attraction.pk])
        response = self.client.post(url)
        self.assertEqual(response.status_code, 302)
        self.assertIn("/accounts/login/", response["Location"])
        self.assertTrue(UserSavedAttraction.objects.filter(user=self.user, attraction=self.attraction).exists())

    def test_authenticated_post_deletes_save(self):
        self.client.force_login(self.user)
        url = reverse("attraction-unsave", args=[self.attraction.pk])
        response = self.client.post(url, HTTP_HX_REQUEST="true")
        self.assertEqual(response.status_code, 200)
        self.assertFalse(UserSavedAttraction.objects.filter(user=self.user, attraction=self.attraction).exists())

    def test_invalid_pk_returns_404(self):
        self.client.force_login(self.user)
        response = self.client.post(reverse("attraction-unsave", args=[99999]), HTTP_HX_REQUEST="true")
        self.assertEqual(response.status_code, 404)

    def test_unsave_when_not_saved_is_idempotent(self):
        self.client.force_login(self.user)
        UserSavedAttraction.objects.filter(user=self.user, attraction=self.attraction).delete()
        url = reverse("attraction-unsave", args=[self.attraction.pk])
        response = self.client.post(url, HTTP_HX_REQUEST="true")
        self.assertEqual(response.status_code, 200)

    def test_htmx_response_contains_save_button(self):
        self.client.force_login(self.user)
        url = reverse("attraction-unsave", args=[self.attraction.pk])
        response = self.client.post(url, HTTP_HX_REQUEST="true")
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, reverse("attraction-save", args=[self.attraction.pk]))

    def test_non_htmx_post_redirects_to_list(self):
        self.client.force_login(self.user)
        url = reverse("attraction-unsave", args=[self.attraction.pk])
        response = self.client.post(url)
        self.assertRedirects(response, reverse("attraction-list"))
...
class SaveAcrossAuthTest(TestCase):
    ...
    def test_register_path_save_persists(self):
        ...
        self.assertTrue(UserSavedAttraction.objects.filter(user=new_user, attraction=self.attraction).exists())
