...
def attraction_list(request):
    ...
    if request.user.is_authenticated:
        ...
        saved_pks = set(
            UserSavedAttraction.objects.filter(user=request.user, attraction_id__in=page_pks).values_list(
    ...
    if request.htmx:
        template = (
            "attractions/partials/cards_append.html" if offset > 0 else "attractions/partials/filter_results.html"
        )
